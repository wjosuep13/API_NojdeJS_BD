            ...
            if (err) reject(err)
                ...
                for (var queryResult of results) {
                    if(resultString=="{\"objeto\":["){
                        ...
                        resultString=resultString+", <br/>"+JSON.stringify(queryResult);
                    }
...
app.get('/:option',function(req,res) {
    ...
    if(op==1){
      getDatabase()
      .then(() => getCollection())
      .then(() => getDocument(config.document1))
      ...
      res.send(op)
        ...
        getDatabase()
        .then(() => getCollection())
        .then(() => getDocument(config.document2))
        .then(() => { exit(`Completed successfully`); })
        .catch((error) => { console.log(`Completed with error ${JSON.stringify(error)}`) });
        res.send(op)
    }
        ...
        getDatabase()
        .then(() => getCollection())
        .then(() => getDocument(config.document3))
        .then(() => { exit(`Completed successfully`); })
        .catch((error) => { console.log(`Completed with error ${JSON.stringify(error)}`) });
        res.send(op)
    }else if(op==4){
        getDatabase()
        .then(() => getCollection())
        .then(() => getDocument(config.document4))
        .then(() => { exit(`Completed successfully`); })
        .catch((error) => { console.log(`Completed with error ${JSON.stringify(error)}`) });
        res.send(op)
    }else if(op==5){
